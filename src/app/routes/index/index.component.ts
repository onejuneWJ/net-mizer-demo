...
export class IndexComponent implements OnInit {
  ...
  loadMap(): void {
    const chartDom = document.getElementById('mapCharts');
    const myChart = echarts.init(chartDom);
    let option;
    this.http.get(`assets/quanzhou.json`).subscribe(geoJson => {
      myChart.hideLoading();
      echarts.registerMap('QZ', geoJson);
        ...
        tooltip: {
          ...
          formatter: (params) => {
            return params.data.name
              + '<br/> 集团用户在线数量：' + formatNumber(params.data.value / 10000, 'zh-CN', '1.0-2') + '万人'
              + '<br/>用户占比：' + formatNumber(params.data.value / 659389.32 * 100, 'zh-CN', '1.0-2') + '%';
          },
        },
        visualMap: {
          min: 800,
        ...
        series: [
          {
            ...
            roam: true,
            data: [
              {name: '鲤城区', value: 20057.34},
              {name: '丰泽区', value: 15477.48},
              {name: '洛江区', value: 31686.1},
              {name: '泉港区', value: 6992.6},
              {name: '惠安县', value: 44045.49},
              {name: '安溪县', value: 40689.64},
              {name: '永春县', value: 37659.78},
              {name: '德化县', value: 45180.97},
              {name: '金门县', value: 55204.26},
              {name: '石狮市', value: 21900.9},
              {name: '晋江市', value: 4918.26},
              {name: '南安市', value: 5881.84}
            ]
          }
        ]
      });
  ...
  dataFormatter(obj): any {
    const pList = ['鲤城区', '丰泽区', '洛江区', '泉港区', '惠安县', '安溪县', '永春县', '德化县', '金门县', '石狮市', '晋江市', '南安市'];
    let temp;
  ...
  loadDk(): void {
    const counties = ['鲤城区', '丰泽区', '洛江区', '泉港区', '惠安县', '安溪县', '永春县', '德化县', '金门县', '石狮市', '晋江市', '南安市'];
    const days = ['2021-02-10', '2021-02-11', '2021-02-12', '2021-02-13', '2021-02-14', '2021-02-15', '2021-02-16'];


    const chartDom = document.getElementById('lmlCharts');
    const myChart = echarts.init(chartDom);
    let option;
  ...
  loadBar(): void {
    const counties = ['鲤城区', '丰泽区', '洛江区', '泉港区', '惠安县', '安溪县', '永春县', '德化县', '金门县', '石狮市', '晋江市', '南安市'];
    const xAxisVal = ['2021年1月', '2021年2月'];
    this.barOptions = {
      title: {
        text: '流量趋势分析',
      },
      ...
      series: [
        {
          ...
          tooltip: {
            ...
            formatter: '{b} <br/> 1月-上行流量: {c} GB'
          },
          data: [18203, 23489, 29034, 14970, 31744, 10230, 18403, 23489, 29034, 24970, 31744, 10230]
        },
        {
          name: '下行',
          stack: '2021年1月',
          ...
          tooltip: {
            ...
            formatter: '{b} <br/> 1月-下行流量: {c} GB'
          },
          data: [18203, 23489, 29034, 14970, 131744, 30230, 18203, 13489, 19034, 34970, 21744, 20230]
        },
        {
          name: '上行',
          stack: '2021年2月',
          ...
          tooltip: {
            ...
            formatter: '{b} <br/> 2月-上行流量: {c} GB'
          },
          data: [19325, 23438, 11000, 21594, 34141, 21807, 28203, 23489, 29034, 18970, 11444, 37230]
        },
        {
          name: '下行',
          stack: '2021年2月',
          ...
          tooltip: {
            ...
            formatter: '{b} <br/> 2月-下行流量: {c} GB'
          },
          data: [19325, 23438, 31000, 11594, 24141, 81807, 15203, 33489, 29034, 10970, 11744, 10230]
        }
      ]
    };
  }
